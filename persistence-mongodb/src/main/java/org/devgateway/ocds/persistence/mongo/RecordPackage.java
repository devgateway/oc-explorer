 ...
 * <p>
 * The record package contains a list of records along with some publishing meta data. The records pull together all the releases under a single Open Contracting ID and compile them into the latest version of the information along with the history of any data changes.
 * 
 */
...
public class RecordPackage implements Identifiable {
	...
	@Override
	public int hashCode() {
		return new HashCodeBuilder().append(uri).append(publisher).append(license).append(publicationPolicy)
				.append(publishedDate).append(packages).append(records).toHashCode();
	}
	...
	public boolean equals(Object other) {
		...
		if ((other instanceof RecordPackage) == false) {
			return false;
		}
		RecordPackage rhs = ((RecordPackage) other);
		return new EqualsBuilder().append(uri, rhs.uri).append(publisher, rhs.publisher).append(license, rhs.license)
				.append(publicationPolicy, rhs.publicationPolicy).append(publishedDate, rhs.publishedDate)
				.append(packages, rhs.packages).append(records, rhs.records).isEquals();
	}
