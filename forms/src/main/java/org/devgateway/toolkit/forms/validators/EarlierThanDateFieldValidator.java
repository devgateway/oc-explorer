...
/**
 * @author mpostelnicu {@link DateFieldBootstrapFormComponent} validator for dates
 *         that have a chronology
 */
public class EarlierThanDateFieldValidator implements IValidator<Date> {
	...
	public void validate(final IValidatable<Date> validatable) {
		...
		if (endDate != null && validatable.getValue() != null && endDate.before(validatable.getValue())) {
			...
			error.setVariable("highDateName", new StringResourceModel(highDate.getLabelKey(), highDate.getParent(),
					null).getString());
