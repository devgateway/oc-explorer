...
public class EditUserPage extends AbstractEditPage<Person>{
    ...
    protected TextFieldBootstrapFormComponent<String> firstName = new TextFieldBootstrapFormComponent<String>("firstName");

    protected TextFieldBootstrapFormComponent<String> lastName = new TextFieldBootstrapFormComponent<String>("lastName");
    ...
    protected Select2MultiChoiceBootstrapFormComponent<Role> roles = new Select2MultiChoiceBootstrapFormComponent<Role>(
            ...
            new GenericPersistableJpaRepositoryTextChoiceProvider<Role>(
                    roleRepository));
    ...
    final protected PasswordFieldBootstrapFormComponent password = new PasswordFieldBootstrapFormComponent("plainPassword");

    final protected PasswordFieldBootstrapFormComponent cpassword = new PasswordFieldBootstrapFormComponent(
        ...
        public void validate(IValidatable<Collection<Role>> validatable) {
            ...
            if(!value.contains(roleAdmin) && value.contains(roleValidator) && !value.contains(roleUser))
                validatable.error( new ValidationError(
                        getString("validatorRoleValidator")));

            if(value.contains(roleAdmin) && (!value.contains(roleValidator) || !value.contains(roleUser)))
                validatable.error( new ValidationError(
                        getString("adminRoleValidator")));
        }
    }
        ...
        public void validate(IValidatable<String> validatable) {
            ...
            for (int i = 0; i < persons.size(); i++) {
                if (persons.get(i) != null
                        && !persons.get(i).getId().equals(userId)) {
                    ValidationError error = new ValidationError(
                            getString("uniqueUser"));
    ...
    protected class UniqueEmailAddressValidator implements IValidator<String> {
        ...
        public UniqueEmailAddressValidator(Long userId) {
            this.userId = userId;
        }
    ...
    protected void onInitialize() {
        ...
        if (!SecurityUtil.isCurrentUserAdmin()) {
            if (person.getId() != getPageParameters().get(
                    WebConstants.PARAM_ID).toLong())
        ...
        StringValue idPerson = getPageParameters().get(
                WebConstants.PARAM_ID);
        if (!idPerson.isNull())
            userName.getField().add(
                    new UniqueUsernameValidator(idPerson.toLong()));
        else
        ...
        editForm.add(roles);
        MetaDataRoleAuthorizationStrategy.authorize(roles, Component.RENDER,
                SecurityConstants.Roles.ROLE_ADMIN);
        ...
        MetaDataRoleAuthorizationStrategy.authorize(enabled, Component.RENDER,
                SecurityConstants.Roles.ROLE_ADMIN);
        ...
        return new SaveEditPageButton("save", new StringResourceModel("save", EditUserPage.this, null)) {
            ...
            protected void onSubmit(AjaxRequestTarget target, Form<?> form) {
                ...
                if (cpass.getField().getModelObject())
                    compoundModel.getObject()
                            .setPassword(
                                    encoder.encode(password.getField()
                                            .getModelObject()));
                if (changePassPage)
                ...
                if(compoundModel.getObject().isChangePass()){
                    ...
                    if(changePassPage){
                        //local test
                        sendEmailService.sendEmailToEnable(compoundModel.getObject(), "http://localhost:8080/changedPasswordValidate?check=");
